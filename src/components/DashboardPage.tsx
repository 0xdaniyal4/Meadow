...
export const DashboardPage: React.FC<DashboardPageProps> = ({
  ...
  const { shortAddress, walletAddress, demoMode } = useAppWallet();
  const [toastMessage, setToastMessage] = useState<string | null>(null);

  // Filter resumes strictly by currently connected wallet address
  const userResumes = resumes.filter((r) => {
    if (!walletAddress) return true;
    if (demoMode) return true;
    return r.authorAddress?.toLowerCase() === walletAddress.toLowerCase();
  });
    ...
    <div className="flex min-h-screen pt-16 w-full">
      ...
      <aside className="hidden md:flex flex-col w-64 fixed left-0 top-16 bottom-0 border-r border-white/10 bg-[#032330] py-6 px-4 z-40">
        ...
        <div className="mb-6 space-y-4">
          <button 
            ...
            className="w-full flex items-center gap-3 text-left hover:bg-white/5 p-2 rounded-xl transition-colors"
          >
            <div className="w-10 h-10 rounded-full bg-[#1c3846] border border-[#66F4FF]/30 flex items-center justify-center text-[#66F4FF] shadow-sm shrink-0">
          ...
          <button 
            ...
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${
              currentTab === 'dashboard' 
                ? 'bg-[#66C4FF]/10 text-[#66C4FF] border-r-4 border-[#66C4FF]' 
                : 'text-[#bec8d1] hover:bg-[#1c3846]/50 hover:text-[#cae7f9]'
            }`}
          >
        ...
        <div className="pt-4 border-t border-white/10 space-y-1">
          <button 
            ...
            className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs text-[#bec8d1] hover:text-[#cae7f9]"
          >
            <ShieldCheck className="w-4 h-4 text-[#66F4FF]" />
          ...
          <a href="https://docs.shelby.xyz" target="_blank" rel="noreferrer" className="flex items-center gap-3 px-3 py-2 rounded-lg text-xs text-[#bec8d1] hover:text-[#cae7f9]">
            ...
            <span>Shelby Protocol Docs</span>
          </a>
        </div>
      </aside>
      ...
      <main className="flex-1 md:ml-64 p-6 lg:p-10 max-w-7xl mx-auto w-full">
        ...
        {userResumes.length === 0 ? (
          <div className="glass-panel rounded-2xl p-12 border border-white/10 bg-[#032330] text-center space-y-4 max-w-xl mx-auto my-12">
            <div className="w-16 h-16 rounded-2xl bg-[#001620] border border-[#66F4FF]/30 text-[#66F4FF] mx-auto flex items-center justify-center shadow-inner">
              <FolderOpen className="w-8 h-8" />
            </div>
            <h3 className="text-xl font-bold text-[#cae7f9]">No resume uploaded yet</h3>
            <p className="text-sm text-[#bec8d1] leading-relaxed">
              Upload your resume document to publish it to Shelby decentralized storage on Aptos.
            </p>
            <button
              onClick={onOpenUpload}
